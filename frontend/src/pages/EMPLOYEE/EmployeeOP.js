...
const EmployeeOrders = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/employee/orders", {
      headers: {
  ...
  const handleMarkAsComplete = (orderId) => {
    fetch(`http://localhost:5000/api/employee/orders/${orderId}/complete`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  const handleCancelOrder = (orderId) => {
    fetch(`http://localhost:5000/api/employee/orders/${orderId}/cancel`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
